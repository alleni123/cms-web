	...
	$(function() {
		...
		var t = $("#tree").mytree({
			...
			callback : {
				...
				//	var parents=getParents(treeNode);
					
				//	console.log(parents);
				//	var childNodes=getChildren(treeNode,_childNodes);
				 
				//	 addGroupChannel(parents);
					 
				//	 addGroupChannel(treeNode.id);
					 
					 //如果该节点没有被选中，则进行添加操作
					 if(!treeNode.checked){
					 	var parents=getCheckedParentsNodes(treeNode, false);
					 	parents.push(treeNode);
					 	addGroupChannel(parents);
					 }
					 else {
					 	var childNodes=new Array();
					 	childNodes=getCheckedChildNodes(treeNode, false, childNodes);
...
<body>
	<div id="content">
		...
		<input type="hidden" id="treePath" value="admin/channel/treeAll" />

...
<body>
	<div id="content">
		...
		<table width="800" cellspacing="0" cellPadding="0">
			<thead>
				<tr>
					<td colspan="2">查询用户功能：用户id[${user.id }]</td>
				</tr>
			</thead>
			<tr>
			...
			<tr>
				...
				<td><c:forEach items="${roles}" var="r">
						<c:if test="${isAdmin}">
							<a href="<%=request.getContextPath()%>/admin/role/show/${r.id}"
								class="list_op"> [${r.name }]</a>&nbsp;
					</c:if>
			...
			<tr>
				...
				<td><c:forEach items="${groups}" var="g">
						<c:if test="${isAdmin}">
							<a href="<%=request.getContextPath()%>/admin/group/show/${g.id}"
								class="list_op">[${g.name}]</a>&nbsp;
					</c:if>
			...
			<tr>
				<c:if test="${isAdmin}">
					<td colspan="2" class="centerTd"><a
						href="admin/user/update/${user.id }" class="list_op">修改用户</a>
					</td>
				</c:if>

				<c:if test="${not isAdmin}">
					<td colspan="2" class="centerTd"><a
						href="<%=request.getContextPath()%>/admin/user/updateSelf"
						target="content">修改个人信息</a>
					</td>
				</c:if>
